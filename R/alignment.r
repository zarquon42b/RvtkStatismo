...
partialAlign <- function(array,use.lm=NULL,scale=FALSE,reference=NULL,reflection=FALSE) {
    if (is.null(reference)){
        if (!is.null(use.lm)){
            ...
            tmp <- lapply(a.list, function(i) {mat <- rotonmat(array[,,i],array[use.lm,,i],procMod$rotated[use.l,,i],scale=scale,reflection = reflection);return(mat)})
        ...
        a.list <-  1:(dim(array)[3])
        tmp <- lapply(a.list, function(i) {mat <- rotonmat(array[,,i],array[use.lm,,i],array[use.lm,,reference],scale=scale,reflection = reflection);return(mat)})
        procMod <- list()
